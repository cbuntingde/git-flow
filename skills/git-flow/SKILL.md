...
license: Apache-2.0
compatibility: Requires git and gh (GitHub CLI) on PATH, with `gh auth status` succeeding for the target repo's host. Targets Modelize >= 1.0 and Claude Code >= 1.0.
metadata:
  modelize:origin: git-flow
allowed-tools: Bash Read Grep
---

# git-flow — branch → PR → wait for green → merge → back to default
...
## Hard rules — read these before anything else

The user pre-announces these on session load (`hooks/hooks.json`
`session_start`), and they are non-negotiable:

- Never force-push the default branch. `git push --force` against
  `<default-branch>` is forbidden. If the user asks for it, abort and
  tell them to do it themselves.
- Never pass `--admin` to `gh pr merge`. If branch protection blocks the
  merge, surface the missing requirement and stop.
- Never merge with red CI. If any required check is failing or pending,
  refuse. Even one failing check blocks the merge.
- Never co-mingle unrelated changes. One task per branch.
- Never invoke `git checkout -- <path>`, `git reset --hard`,
  `git stash drop` / `git stash clear`, or `git clean -fd` mid-workflow
  without explicit user approval (silent revert). See rule 11 in
  `references/safety.md`.
- Never start a new branch while the local default branch has unpushed
  commits. Push, rebase, or drop them first. See rule 12 in
  `references/safety.md`.
- When Actions is unavailable for the repository, fall back to the
  step 2.5 local check as the merge gate — never skip the gate.
...
- The user invokes a slash command from this plugin (`/git-flow:branch`,
  `/git-flow:pr`, `/git-flow:watch`, `/git-flow:merge`,
  `/git-flow:status`, `/git-flow:back-to-main`).
...
# 0d. Detect the default branch (do NOT assume "main"). MODELIZE_GIT_FLOW_BASE_BRANCH
# overrides detection. An empty value must abort here — it would make every
# later ref expansion (`origin/`) meaningless.
DEFAULT_BRANCH="${MODELIZE_GIT_FLOW_BASE_BRANCH:-$(gh repo view --json defaultBranchRef -q .defaultBranchRef.name 2>/dev/null || true)}"
if [ -z "$DEFAULT_BRANCH" ]; then
  echo "aborted: could not determine the default branch. Set MODELIZE_GIT_FLOW_BASE_BRANCH and retry."
  exit 1
fi
...
`MODELIZE_GIT_FLOW_SKIP_LOCAL_CHECK=1`.
...
```bash
timeout "${MODELIZE_GIT_FLOW_WATCH_TIMEOUT_MIN:-30}m" \
...
# did not ask for.
case "${MODELIZE_GIT_FLOW_MERGE_STRATEGY:-squash}" in
  squash) STRATEGY=--squash ;;
  rebase) STRATEGY=--rebase ;;
  merge)  STRATEGY=--merge  ;;
  *)
    echo "aborted: MODELIZE_GIT_FLOW_MERGE_STRATEGY must be squash, rebase, or merge (got '${MODELIZE_GIT_FLOW_MERGE_STRATEGY}')."
    exit 1
    ;;
esac

if [ "${MODELIZE_GIT_FLOW_DELETE_REMOTE_BRANCH:-0}" = "1" ]; then
  gh pr merge "$STRATEGY" --delete-branch --body "Merged by git-flow"
else
  gh pr merge "$STRATEGY" --body "Merged by git-flow"
fi
```

`--squash` is the default. The user can override via
`MODELIZE_GIT_FLOW_MERGE_STRATEGY=rebase` or `=merge`. See
`references/merge-strategy.md` for the decision tree.

The remote branch (`<branch>` on `origin`) is **kept by default** after
merge. Set `MODELIZE_GIT_FLOW_DELETE_REMOTE_BRANCH=1` to pass `--delete-branch`
...
# MODELIZE_GIT_FLOW_DELETE_REMOTE_BRANCH=1 was set (handled in step 5).
...
|---|---|
| `/git-flow:branch <slug>` | 0, 1 |
| `/git-flow:pr` | 3 |
| `/git-flow:watch` | 4 |
| `/git-flow:merge` | 5, 6 |
| `/git-flow:status` | read-only — print current branch, PR URL, check states |
| `/git-flow:back-to-main` | 6 only (no merge; abandons current branch) |
| `/git-flow:setup-ci` | one-shot bootstrapper — drafts `.github/workflows/ci.yml`, requires user approval, opens a PR. Outside the 0→7 procedure. |
...
| Var | Default | Effect |
|---|---|---|
| `MODELIZE_GIT_FLOW_WATCH_TIMEOUT_MIN` | `30` | Timeout for `gh pr checks --watch`. |
| `MODELIZE_GIT_FLOW_MERGE_STRATEGY` | `squash` | `squash` / `rebase` / `merge`. |
| `MODELIZE_GIT_FLOW_DELETE_REMOTE_BRANCH` | `0` | When `1`, pass `--delete-branch` to `gh pr merge`. Default `0` keeps the remote branch. |
| `MODELIZE_GIT_FLOW_BASE_BRANCH` | _(auto-detect from `gh`)_ | Override default branch. |
| `MODELIZE_GIT_FLOW_SKIP_LOCAL_CHECK` | `0` | When `1`, skip step 2.5 entirely. |
| `MODELIZE_GIT_FLOW_LOCAL_CHECK_TIMEOUT` | `300` | Per-check timeout in seconds for step 2.5. |

## State persistence

After every successful step, write
`.git/git-flow/state.json` with the schema in
...
`.git/git-flow/local-check.json` (see `local-check.md`). State
...
- `references/setup-ci.md` — `/git-flow:setup-ci` scaffold logic.
