...
8. **No `--force` on `--delete-branch`.** Deleting a remote branch that
   ...
   The `/git-flow:back-to-main --delete-remote` flag is the
...
```
git-flow: branch=<branch> base=<default> strategy=<squash|rebase|merge> timeout=<n>m
```

Every workflow run prints, at the end (success or failure):

```
git-flow: result=<merged|aborted|timeout> pr=<#n|url> checks=<green|red|timeout> branch=<branch>
```

The user can grep `~/.modelize/logs/modelize.log` for `git-flow:`
