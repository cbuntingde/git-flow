...
`/git-flow:pr` again to push a follow-up, or `/merge` re-reads
...
- `MODELIZE_GIT_FLOW_SKIP_LOCAL_CHECK=1` → skip silently.
...
`MODELIZE_GIT_FLOW_LOCAL_CHECK_TIMEOUT` (in seconds). Long enough for a
...
`.git/git-flow/local-check.json` (gitignored). Schema:
...
This file is read by `/git-flow:status`, `/git-flow:watch`,
and `/git-flow:merge` so the user can see what was checked even
