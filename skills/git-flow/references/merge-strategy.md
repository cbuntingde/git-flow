...
Default: **squash**. Override via `MODELIZE_GIT_FLOW_MERGE_STRATEGY`.
...
1. Read `MODELIZE_GIT_FLOW_MERGE_STRATEGY`. If set to `squash`, `rebase`, or
   `merge`, use that.
...
gh pr merge --squash --body "Merged by git-flow"

# Override: rebase — remote branch is kept.
gh pr merge --rebase --body "Merged by git-flow"

# Override: merge commit — remote branch is kept.
gh pr merge --merge --body "Merged by git-flow"

# Opt back into remote deletion (any strategy).
MODELIZE_GIT_FLOW_DELETE_REMOTE_BRANCH=1 gh pr merge --squash --delete-branch --body "Merged by git-flow"
```

By default the remote branch is **not** deleted. Pass
`MODELIZE_GIT_FLOW_DELETE_REMOTE_BRANCH=1` to add `--delete-branch` to whichever
...
- **To opt back into remote deletion**, set
  `MODELIZE_GIT_FLOW_DELETE_REMOTE_BRANCH=1` before invoking
  `/git-flow:merge`. The workflow passes `--delete-branch` to
