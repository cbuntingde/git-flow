...
This is the logic behind `/git-flow:setup-ci`. It detects the
...
main 0→7 procedure; the user must invoke `/git-flow:setup-ci`
...
`/git-flow:watch` and `/git-flow:merge` to land it.
