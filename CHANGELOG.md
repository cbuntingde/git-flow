...
- **Renamed to `git-flow` and migrated from Kimi Code CLI to Modelize.**
  The plugin was originally `kimi-git-flow` and lived at `kimi.plugin.json`,
  which Modelize does not read. It now ships as `git-flow` with the
  standard `plugin.json` manifest Modelize expects. Everything user-visible
  was renamed in lockstep: the slash commands are now `/git-flow:*`, the
  env vars are `MODELIZE_GIT_FLOW_*` (no more `KIMI_GIT_FLOW_*`), the
  per-branch state files live under `.git/git-flow/` (was
  `.git/kimi-git-flow/`), the `gh pr merge` body reads `Merged by git-flow`,
  and the audit-trail prefix is `git-flow:` (was `kimi-git-flow:`).
  The `sessionStart.skill` field the legacy manifest carried is replaced
  by a `hooks/hooks.json` `session_start` hook that reminds the model to
  load the skill on session start. Old per-branch state files from a
  previous Kimi Code install are not migrated — the values are
  gitignored and a fresh run regenerates them.

### Added

- **Claude Code support.** A sibling `.claude-plugin/plugin.json`
  ships alongside `plugin.json` so the same directory loads on both
  Modelize and Claude Code. The shared `hooks/hooks.json` declares both
  `session_start` (Modelize) and `SessionStart` (Claude Code) so the
  session-start reminder fires in either editor. Cursor and GitHub
  Copilot also read `plugin.json` from the same directory. OpenAI
  Codex is not supported (no plugin format); see README for the
  manual `instructions.md` workaround.
