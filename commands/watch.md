...
Run step 4 of the git-flow procedure: wait for required CI checks
...
Optional env var: `MODELIZE_GIT_FLOW_WATCH_TIMEOUT_MIN` (default `30`).
...
2. Run `timeout "${MODELIZE_GIT_FLOW_WATCH_TIMEOUT_MIN:-30}m" gh pr checks --watch --interval 30`.
3. On exit 0 → all checks green. Print "checks green for PR #<n>" and
   stop. The user runs `/git-flow:merge` next.
...
`.git/git-flow/local-check.json` on the current branch.
...
- No PR is open. Use `/git-flow:pr` first.
...
- `/git-flow:merge` — merge the PR once green.
