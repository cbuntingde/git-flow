...
Run steps 5 and 6 of the git-flow procedure: merge the PR and
...
```text
/git-flow:merge [--dry-run]
```

Optional env vars:

- `MODELIZE_GIT_FLOW_MERGE_STRATEGY` — `squash` (default), `rebase`, or `merge`.
- `MODELIZE_GIT_FLOW_DELETE_REMOTE_BRANCH` — `1` to pass `--delete-branch` to
  `gh pr merge`; default `0` keeps the remote branch.
- `MODELIZE_GIT_FLOW_BASE_BRANCH` — override the detected default branch.
...
1. Confirm there is an open PR for the current branch and it is
   ...
   ```
   aborted: no pull request for current branch <name>. Run /git-flow:pr first.
   ```

   Do not silently swallow the `gh` failure — the user needs the
   exact recovery command.
2. Read the local-check result for the current branch from
   `.git/git-flow/local-check.json`. Verify the recorded `branch`
   ...
   `MODELIZE_GIT_FLOW_SKIP_LOCAL_CHECK=1` is set. If the recorded
...
4. `gh pr merge --<strategy> [--delete-branch] --body "Merged by git-flow"`.
   Default strategy is `squash`. `--delete-branch` is appended only when
   `MODELIZE_GIT_FLOW_DELETE_REMOTE_BRANCH=1`; otherwise the remote branch
   is kept.
...
- The local check (step 2.5) failed on this branch — the result is
  visible in `/git-flow:status`. Fix the failing check, push a
