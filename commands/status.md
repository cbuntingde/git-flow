...
6. **Local check (step 2.5)**: the recorded result for this branch from
   `.git/git-flow/local-check.json`, printed as
...
8. **Last action**: the most recent step this workflow ran on the current
   branch. Read from `.git/git-flow/state.json` (gitignored).
...
- `/git-flow:branch`, `/git-flow:pr`, `/git-flow:watch`,
  `/git-flow:merge`, `/git-flow:back-to-main` — the
