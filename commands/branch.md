...
# /git-flow:branch <slug>

Run steps 0 and 1 of the git-flow procedure and stop. Leaves the
user on the new branch with a clean working tree.

## Usage
```text
/git-flow:branch <slug> [--dry-run]
```
...
- The user wants to push and open a PR right now. Use
  `/git-flow:pr` after `/git-flow:branch` instead.
...
- `/git-flow:pr` — push and open the PR for the current branch.
