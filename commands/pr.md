...
Run step 3 of the git-flow procedure: push and open a PR for the
...
3. Run step 2.5 — the local check — against the working tree if the
   ...
   recorded in `.git/git-flow/local-check.json` for `/status`,
   `/watch`, and `/merge` to read. See
   `skills/git-flow/references/local-check.md`. Skip when
   `MODELIZE_GIT_FLOW_SKIP_LOCAL_CHECK=1`. This step is always part of
...
- The PR is already open. Use `/git-flow:status` to inspect; use
  `/git-flow:watch` to wait for green.
...
- `/git-flow:watch` — wait for CI on the just-opened PR.
- `/git-flow:status` — see the PR state without doing anything.