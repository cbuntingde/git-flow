// Smoke test for git-flow plugin integrity.
//
...
function parseFrontmatter(md) {
  ...
  m[1].split(/\r?\n/).forEach((line, i) => {
    ...
    // (e.g. `modelize:origin`), so the key group is non-greedy.
...
test("plugin.json is well-formed and exposes the host-runtime keys", () => {
  const cfg = JSON.parse(read("plugin.json"));
  assert.equal(cfg.name, "git-flow");
  assert.ok(cfg.version, "version must be set");
  assert.equal(cfg.skills, "./skills/");
  assert.equal(cfg.commands, "./commands/");
  assert.ok(typeof cfg.hooks === "string" || Array.isArray(cfg.hooks), "hooks must be a string path or an array");
  // Modelize only reads a small set of fields from the manifest. The rest are
  // documentation, not contract — guard against drift on the documented ones.
  const allowed = new Set(["name", "version", "description", "homepage", "repository", "documentation", "skills", "commands", "hooks", "interface", "license"]);
  for (const key of Object.keys(cfg)) {
    assert.ok(allowed.has(key), `plugin.json uses unknown key ${JSON.stringify(key)} — Modelize ignores it; remove or document it`);
  }
});

test("SKILL.md hard-rules block references the non-negotiable safety rules", () => {
  // Modelize loads the SKILL.md body itself; the safety reminder that lived
  // in the legacy manifest's `skillInstructions` moved into the skill body.
  const skill = read(SKILL_FILE);
  const rulesBlock = skill.split(/^## Hard rules/m)[1]?.split(/^## /m)[0] ?? "";
  assert.ok(rulesBlock.length > 0, "SKILL.md must have a `## Hard rules` block carrying the non-negotiable rules");
  const rules = [
  ...
  for (const r of rules) {
    assert.ok(r.re.test(rulesBlock), `SKILL.md Hard rules must mention: ${r.name}`);
  }
});

test(".claude-plugin/plugin.json exists, parses, and agrees with plugin.json on the load-bearing fields", () => {
  const claudePath = ".claude-plugin/plugin.json";
  assert.ok(exists(claudePath), `${claudePath} must exist so Claude Code can discover the plugin`);

  const modelize = JSON.parse(read("plugin.json"));
  const claude = JSON.parse(read(claudePath));

  // The two manifests identify the same plugin. Drift here is the #1 source
  // of "works on Modelize but missing on Claude Code" reports.
  for (const key of ["name", "version", "skills", "commands", "hooks"]) {
    assert.equal(
      claude[key],
      modelize[key],
      `${claudePath}'s ${JSON.stringify(key)} must agree with plugin.json (${JSON.stringify(modelize[key])} vs ${JSON.stringify(claude[key])})`,
    );
  }
  assert.equal(claude.license, modelize.license, "license must agree across both manifests");

  // Both manifests use the same allowlist of fields — drift towards an
  // editor-only field would silently break the other client.
  ...
  for (const key of Object.keys(claude)) {
    assert.ok(allowed.has(key), `${claudePath} uses unknown key ${JSON.stringify(key)} — remove or document it`);
  }
});

test("hooks/hooks.json declares both session_start (Modelize) and SessionStart (Claude Code)", () => {
  const hooks = JSON.parse(read("hooks/hooks.json"));
  // Modelize uses snake_case; Claude Code uses PascalCase. Both clients look
  // up by event name and silently skip unknown ones, so shipping both is
  // the only way to fire the session-start reminder on either editor.
  assert.ok(hooks.hooks?.session_start, "hooks/hooks.json must declare `session_start` for Modelize");
  assert.ok(hooks.hooks?.SessionStart, "hooks/hooks.json must declare `SessionStart` for Claude Code");

  for (const event of ["session_start", "SessionStart"]) {
    const entries = hooks.hooks[event];
    assert.ok(Array.isArray(entries) && entries.length > 0, `${event} must have at least one matcher entry`);
    for (const entry of entries) {
      assert.ok(entry.hooks && entry.hooks.length > 0, `${event} entry must declare a non-empty hooks[] array`);
      for (const h of entry.hooks) {
        assert.equal(h.type, "command", `${event} hook must be of type "command": ${JSON.stringify(h)}`);
        assert.ok(typeof h.command === "string" && h.command.length > 0, `${event} hook command must be a non-empty string`);
      }
    }
  }
});
...
test("frontmatter parses as valid YAML (host-rejectable constructs are rejected)", () => {
  ...
  assert.doesNotThrow(
    () => parseFrontmatter("---\nname: x\ndescription: y\nmetadata:\n  modelize:origin: git-flow\n---\n"),
    "a `modelize:origin` nested key must be accepted",
  );
  assert.throws(
    () => parseFrontmatter("---\nname: x\n---\n"),
...
test("SKILL.md step 5 honors MODELIZE_GIT_FLOW_MERGE_STRATEGY instead of hardcoding --squash", () => {
  const step5 = read(SKILL_FILE).split("### 5. Merge")[1].split("### 6.")[0];
  assert.ok(/MODELIZE_GIT_FLOW_MERGE_STRATEGY/.test(step5), "step 5 must read MODELIZE_GIT_FLOW_MERGE_STRATEGY");
...
test("watch.md and merge.md surface /git-flow:pr when no PR is open", () => {
  for (const rel of ["commands/watch.md", "commands/merge.md"]) {
    assert.ok(
      /\/git-flow:pr/.test(read(rel)),
      `${rel} must mention /git-flow:pr as the recovery path when no PR is open`,
    );
  }
});
